...
.PHONY: archive
archive: build
	xcodebuild archive -scheme "SwiftBinaryPackage-Package" \
		-sdk iphoneos \
		-archivePath ".build/artifacts/ios.xcarchive" \
		SKIP_INSTALL=NO
	xcodebuild archive -scheme "SwiftBinaryPackage-Package" \
		-sdk iphonesimulator \
		-archivePath ".build/artifacts/ios-sim.xcarchive" \
		SKIP_INSTALL=NO
	xcodebuild archive -scheme "SwiftBinaryPackage-Package" \
		-sdk macosx \
		-archivePath ".build/artifacts/macos.xcarchive" \
		SKIP_INSTALL=NO

	xcodebuild -create-xcframework \
		-framework ".build/artifacts/ios.xcarchive/Products/Library/Frameworks/SwiftBinaryPackage.framework" \
		-framework ".build/artifacts/ios-sim.xcarchive/Products/Library/Frameworks/SwiftBinaryPackage.framework" \
		-framework ".build/artifacts/macos.xcarchive/Products/Library/Frameworks/SwiftBinaryPackage.framework" \
		-output ".build/artifacts/binary.xcframework"
